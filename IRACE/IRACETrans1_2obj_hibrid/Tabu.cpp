...
void include(int origem, int destino, list <tabuItem> &tabu_list){
	...
	if (ha==false){
		tabuItem item = {origem, destino, tabutenuree};
...
bool buscaTabu(SolucaoEdgeSet *s, double lambda[NUMOBJETIVOS]){
	...
	do{ // repita até contIteracoes==max_tabuu or s nao seja modificado
		...
		} while (r<max_vizinhoss && s_was_modified == false);
		for (list <tabuItem>::iterator it = tabu_list.begin(); it!=tabu_list.end(); it++){
			(*it).tempo -=1;
			if ((*it).tempo==0) {
		...
		contIteracoes++;
	}while (contIteracoes<max_tabuu && s_was_modified == true);

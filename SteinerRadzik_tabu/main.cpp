...
# The second phase uses a Local Search with a "tabu list" of moviments
#=======================================================================
*/


//TODO: verificar se a busca TABU está ok ou se nao, aplicar a busca tabu de rocha et al 
//TODO: quando retornar o conjunto de vizinhanças, pensar em coloca um for anterior que deixa somente os vizinhos nao dominados entre si. Depois coloca o for entre os vizinhos e as solucoes parciais
//TUDO: dadas duas solucoes a e b em triângulos diferentes. Entao a e b nao podem ser dominadas entre si. Ou seja, nem a dominada b, nem b domina a.
		//usar esse fato pra diminuir a quantidade de verificacoes de dominância
...
list < pair<int*, pair<float, float> > >  phase2KB(Grafo *g, list< pair<int*, pair<float, float> > > extremas){
	...
	while (contador<size-1){
		...
		int *q = pq.first;
		if (contador == 0){
			localSearch(g, pp,MSTs);
			localSearch(g, pq,MSTs);
		} else {
			localSearch(g, pq,MSTs);
		}
		...
		for (; k<MSTs.size(); k++){
			...
			if (isInViableRegion(g, regiaoViavel, x, y)){
				...
				pair<int*, pair<float, float> >  pert;
				localSearch(g, k_best,MSTs);
				MSTs.sort(compare);

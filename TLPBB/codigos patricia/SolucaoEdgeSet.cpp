...
class SolucaoEdgeSet : public Solucao {
	...
	/* Faz um caminho aleatorio no grafo G e gera uma arvore
	 * Complexidade (esperada) O(n lg n) */
	void doRandomWalk() {
		bool usados[NUMEROVERTICES];
		memset(usados,0,sizeof(usados));
		int v = rg->IRandom(0,NUMEROVERTICES-1), viz;
		usados[v] = true;
		int cont = 0;

		while (cont < NUMEROVERTICES-1) {
			viz = rg->IRandom(0,g->graus[v]-1);
			if (!g->completo) viz = g->listaadj[v][viz];
			if (!usados[viz]) {
				edges[cont][0] = v;
				edges[cont][1] = viz;
				cont++;

...
void input(char *arq) {
	...
	D(printf("1. Iniciando leitura dos custos\n");)
		...
		char tmp[200];
	fscanf(file,"%d",&nv);
	if (nv != NUMEROVERTICES) {
		printf("ERRO NA INSTANCIA! ESSE EXECUTAVEL SERVE PARA %d NOS E A INSTANCIA EH DE %d\n",NUMEROVERTICES,nv);
		exit(0);
	}
	...
	for (int i=0;i<NUMEROVERTICES-1;i++) {
		for (int j=i+1;j<NUMEROVERTICES;j++) {
			if (NUMEROVERTICES < 1000) fscanf(file,"%d %d %lf %lf",&k,&l,&custos[0][i][j], &custos[1][i][j] );
...
int main( int argc,  char *argv[]) {
	...
	if (argc < 4) {
		printf("Arvore Geradora Minima Multi-Criterio - Algoritmo de Testes. Utilizacao: %s (arquivo da instancia) (semente aleatoria) (arquivo lixo global)\n\n\n",argv[0]);
		return 1;
	}

...
/* Complexidade: O(n) ARQUIVO LIMITADO */
bool Arquivo::adicionarSol(pair<int*, pair<float, float> > s) {
	//cout<<"Antes = "<<s.second.first<<" "<<s.second.second<<endl;
	
	list<pair<int*, pair<float, float> > >::iterator maisPopuloso = soluces.begin();
	int maiorPositionCount = -1;
	if (soluces.size() > 0) maiorPositionCount = getPositionCount( *soluces.begin() );
	...
	while (it != soluces.end()) {
		...
		// se a solucao que vai entrar nao domina a solucao it, procuramos a solucao que esta no
		// local mais populoso, caso o pareto esteja no tamanho maximo
		if (remover.size() == 0 && soluces.size()+1 > MAXARCSIZE) {
			int positionCountAtual = getPositionCount(*it);
...
/* Complexidade: O(n) */
bool Arquivo::adicionarSol22(pair<int*, pair<float, float> > s) {
	//cout<<"Antes = "<<s.second.first<<" "<<s.second.second<<endl;
	list<pair<int*, pair<float, float> > >::iterator it = soluces.begin();
	list< list<pair<int*, pair<float, float> > >::iterator > remover; // uma lista de interators
	while (it != soluces.end()) {

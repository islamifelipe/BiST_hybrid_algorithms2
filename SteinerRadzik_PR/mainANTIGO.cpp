...
#define maxSizePath 10 // 13 é um bom valor (a priori)
...
*/
list <pair<int*,  pair<float, float> > > vizinhos(Grafo *g, pair<int*, pair<float, float> > sol, float cateto_x, float cateto_y){
...
/*Quantidade de arestas que existem em xt (target), mas que nao existem em xi. 
...
void path_relinking(Grafo *g, pair<int*, pair<float, float> > x_startaux, pair<int*, pair<float, float> > x_target, list <pair<int*, pair<float, float> > > &solucoes, float cateto_x, float cateto_y, float escalarX, float escalarY, int maxSolPorTriangulo){
	...
	do {
		...
		if (delta>=1 && cont<maxSizePath && solucoes.size()<maxSolPorTriangulo){
			//cout<<"cont = "<<cont<<endl;
			list <pair<int*,  pair<float, float> > > viz = vizinhos(g, x_start,cateto_x, cateto_y); // somente os vizinhos dentro do triangulo formado pelos catetos x,y
			float min = INT_MAX;
			int min_delta = delta;
			...
			for (list<pair<int*, pair<float, float> > >::iterator viz_it=viz.begin(); viz_it!=viz.end(); viz_it++){ // coloca os vizinhos em retorno
				bool ha = false;
				// retira os vizinhos dominados entre si
